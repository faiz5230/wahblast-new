    ...
    <div class="row align-items-center">
        ...
        <div class="col-6">
            <div class="text-end upgrade-btn">
                <a href="{{ route('admin.add-schedule-group-message') }}" class="btn btn-primary text-white"><i class="mdi mdi-pencil-box-outline"></i> Create schedule group message</a>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="row">
        <div class="card">
                <div class="card-body">
                        <div class="d-md-flex">
                        <div>
                            <h4 class="card-title">Schedule Group Message</h4>
                            <hr width="100px">
                        </div>
                    </div>
                    <!-- title -->
                                ...
                                @if (!empty($dataSchedules) && $dataSchedules->count())
                                    @foreach ($dataSchedules as $dataSchedule)
                                        <tr>
                                            ...
                                            <td>
                                            <a href="{{ route('admin.delete-schedule-group-message', [$dataSchedule->id]) }}" onclick="return confirm('Apakah anda yakin ingin menghapus schedule group message?')" class="{{ $dataSchedule->status == 1 ? 'btn btn-danger disabled' : 'btn btn-danger' }}"><i class="mdi mdi-delete"></i></a>
                                            <a href="{{ route('admin.edit-schedule-group-message', [$dataSchedule->id]) }}" class="{{ $dataSchedule->status == 1 ? 'btn btn-warning disabled' : 'btn btn-warning' }}"><i class="mdi mdi-border-color"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
