                        ...
                        <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                            ...
                            <div class="table-responsive">
                                <table class="table mb-0 table-hover align-middle text-nowrap table-bordered" id="tableID">
                                    ...
                                    <tbody>
                                        <?php $i=1; ?>
                                        @if (!empty($groupChatLogs) && $groupChatLogs->count())
                                            @foreach ($groupChatLogs as $groupChatLog)
                                                <tr>
                                                    <td>{{ $i++ }}</td>
                                                    <td>{{ $groupChatLog->number }}</td>
                                                    <td>{{ $groupChatLog->text }}</td>
                                                    @if($groupChatLog->status == 1)
                                                    <td><label for="status" class="badge bg-success">Success</label></td>
                                                    @else
                                                    <td><label for="status" class="badge bg-danger">Failed</label></td>
                                                    @endif
                                                    <td>{{ $groupChatLog->id_device }}</td>
                                                    <td>{{ $groupChatLog->type }}</td>
                                                    {{-- <td> --}}
